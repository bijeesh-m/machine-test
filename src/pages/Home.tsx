...
const Home = () => {
  ...
  return (
    <div
      className={` w-full min-h-screen bg-gray-300 flex flex-col justify-center items-center`}
    >
      <div className=" w-2/3 my-4  flex justify-between  ">
        <input
          onChange={(e) => setSearchQuery(e.target.value)}
          className=" w-2/3 h-10 focus:outline-none rounded-sm px-2"
          type="text"
      ...
      <div
        className={`${
          userForm && "blur-sm"
        } w-2/3 min-h-[80vh] bg-white container  rounded-lg p-3 mb-10`}
      >
            ...
            ? filterData
                ...
                .map((user) => {
                  return (
                    ...
                    >
                      <div
                        className=" w-10 p-3 rounded-full h-10 bg-cover"
                        style={{ backgroundImage: `url(${user.avatar})` }}
                      />
                      <div className=" w-1/4 p-3 flex  items-center text-sm">
                      ...
                      <div className=" w-1/4 p-4 flex  items-center">
                        <button
                          ...
                          className=" bg-yellow-300 rounded-md flex py-1 px-2"
                        >
                          <svg
        ...
        <div
          className={` ${
            users.length ? "block" : "hidden"
          } mt-5  w-full flex justify-between`}
        >
          <button
            disabled={page === 1 ? true : false}
            onClick={handlePrevPage}
            className={` py-1 px-2 bg-black rounded-sm text-white ${
              page === 1 && "bg-gray-200"
            }`}
          >
            Prev
      ...
      {userForm && (
        <div ref={userFormRef} className="absolute">
          <form
            onSubmit={(e) => handleSaveUser(e, user.id)}
            className=" flex flex-col w-full  bg-green-200 border border-black p-10 space-y-10 rounded-md "
          >
